...
```bash
cargo install --git https://github.com/patractlabs/cargo-contract --branch v0.12.1 --force
```
...
```bash
git clone https://github.com/patractlabs/cargo-contract --branch v0.12.1
cd cargo-contract
cargo build --release
```
...
#### 3. Special rpc interface

After europa v0.3, we provide some special rpc. Those rpc is mainly used for "Contract Tracing" feature.

Before europa v0.2, though we record all necessary information in Contract Tracing, turning the execution of contract
from black box to white box. However, this information just can be printed in console, not be persistent in somewhere.

In v0.3, we store this information in local and provide some rpc for user to get it.

1. `contractsExt_tracing` (params: \[`height: NumberOf<B>`, `index: u32`\])

    This rpc is used for returning the contract tracing information for an extrinsic. So this rpc params is `number` and `index`
    to index an extrinsic.
   
    The return type is following, the "trace" part is the json type for "Contract Tracing" which is printed in console log:
    ...
    "trace": {
            ...
            "env_trace": [
                {
                    "SealInput": {
                        "buf": "0x9bae9d5e40420f00000000000000000000000000"
                    }
                },
            ]
        }
